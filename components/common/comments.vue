<template>
  <div class="comment-box" id="comment-box" :class="{ mobile: mobileLayout }" v-click-outside="clickOut">
    <div class="tools">
      <div class="total">
        <strong class="count">{{ comment.data.data.size || 0 }}</strong>
        <span>&nbsp;</span>
        <span>条评论</span>
      </div>
      ...
      <div class="editor-box">
        ...
        <div class="editor">
          <transition-group tag="div" name="list">
            <div class="will-reply" v-if="!!parentId" key="1">
    ...
    <transition-group name="list" tag="span">
      <!--div class="empty-box" v-if="!comment.data.data || !comment.data.data.length" key="1">暂无评论</div-->
      <div class="list-box" v-if="comment.data && comment.data.data && comment.data.data.list && comment.data.data.list.length > 0" key="1">
        <transition-group name="list" tag="ul" class="comment-list">
          <li
              v-for="comment in comment.data.data.list"
            ...
            <div class="cm-avatar" v-if="!mobileLayout">
              <a target="_blank"
                 ...
                 :href="comment.url"
                 @click.stop="clickUser($event, comment.name)">
                <img v-if="comment.gravatar" :title="comment.name || '匿名用户'"
                  :src="comment.gravatar" />
                <img v-else :alt="comment.name || '匿名用户'"
            ...
            <div class="cm-body">
              <div class="cm-header">
                ...
                <span class="flool">{{ comment.createDate | dateFormat('yyyy-MM-dd hh:mm')}}</span>
              </div>
              <div class="cm-content">
                <div class="reply-box" v-if="!!comment.parentId">
                  <p class="reply-name">
                    <a href="" @click.stop.prevent="toSomeAnchorById(`comment-item-${comment.parentId}`)">
                      <span></span>
                      <strong v-if="fondReplyParent(comment.parentId)">{{ fondReplyParent(comment.parentId) }}</strong>
                    </a>
                  </p>
                  <div 
                  class="reply-content"
                  v-html="fondReplyParentContent(comment.parentId).length > 150
                  ? fondReplyParentContent(comment.parentId).slice(0, 150) + '...'
                  : fondReplyParentContent(comment.parentId)" ></div>
                </div>
              ...
              <div class="cm-footer">
                <a href="" 
                  ...
                  <span>赞&nbsp;({{ !comment.likeCount && comment.likeCount == null ? 0 : comment.likeCount }})</span></a>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      replyCommentSlef() {
        return this.comment.data.data.list.find(comment => Object.is(comment.id, this.parentId))
      },
      ...
      haveMore () {
        return this.$store.state.comment.data.pageNum
                !== this.$store.state.comment.data.pages
      }
    },
    ...
    mounted () {
      ...
      window.onscroll = _.throttle(() => {
        ...
        if (scrollHeight -  scrolleTop - windowHeight <= 200) {
          const {pageNum} = this.comment.data.data

          if (!this.comment.data.data.pageNum && !this.comment.fetching) {
            this.loadComemntList()
          } else if (this.haveMore && !this.comment.fetching) {
            this.loadComemntList({
              currentPage: this.comment.data.data.pageNum + 1
            })
          }
        }
      }, 800)
    ...
    methods: {
      ...
      updateUserCache(event) {
        ...
        if (this.user.url && !this.regexs.url.test(this.user.url)) return alert('链接不合法')
      ...
      clickUser(event, user) {
        if (!user.url) event.preventDefault()
      },
      ...
      // 找到回复来源
      fondReplyParent(parentId) {
        const parent = this.comment.data.data.list.find(comment => Object.is(comment.id, parentId))
        return parent ? parent.name : null
      },

      // 回复来源内容
      fondReplyParentContent (parentId) {
        const parent = this.comment.data.data.list.find(comment => Object.is(comment.id, parentId))
         if(!parent) {
          return ""
        }
      ...
      likeComment(comment) {
        ...
        this.$store.dispatch('comment/likeComment', { type: 1, id: comment.id })
        .then(data => {
      ...
      async loadComemntList(params = {}) {
        ...
        params.sort = -1
        const res = await this.$store.dispatch('comment/loadCommentsByRefId', {
          ...params,
          refId: this.refId
        })
      },
