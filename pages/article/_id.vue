...
export default {
  ...
  computed: {
    ...
    keywords() {
      var articleTagArray = this.$store.state.article.details.tagArray

      if(articleTagArray && articleTagArray.length > 0) {
    ...
    articleContent () {
      var html = markdown(this.article.content, false, true).html

      if(html.indexOf("@[toc]") > 1 && this.toc) {
        return html.replace("@[toc]" , this.toc)
      }
      return html
    },
    ...
    isLiked () {
      return this.likeArticles.includes(this.article.url)
    },
  ...
  methods: {
    async like () {
      ...
      const res = await this.$store.dispatch('article/likeArt', { id: this.article.url })
      if (res.status !== 1) 
        alert(`点赞文章失败：${res.message}`)
      else {
        //this.$store.state.options.option.likeCount = res.data
        this.likeArticles.push(this.article.url)
