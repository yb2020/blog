...
export function getHero(params) {
  return ax.get(serviceId + "/hero", { params }).then(res => res.data);
}

// 增加英雄榜
export function postHero(data) {
  return ax.post("/hero", { ...data }).then(res => res.data);
}
...
export function getMusicLst(data) {
  return ax.get("/music/list/962562705").then(res => res.data);
}

// 音乐详情
export function getMusicDetail(para) {
  return ax.get(`/music/song/${para.song_id}`).then(res => res.data);
}

// 音乐地址
export function getMusicUrl(para) {
  return ax.get(`/music/url/${para.id}`).then(res => res.data);
}

// 获取书本
export function getBook(params) {
  return ax.get(`/book`, { params }).then(res => res.data);
}
